% Example propagation of coherent light showing extreme conditions:
% sampling distance close to wavelength, highly curved boundary of 
% aliased parts of convolution kernels.
%
...
% saved (in case displayKenels is true) to 
...
case 0
  ...
  sourceDescription = sprintf('Point sources at [%.1f, %.1f, %.1f] and [%.1f, %.1f, %.1f] [um]', ...
    lightSource1(1)*1e6, lightSource1(2)*1e6, sourceCenter(3)*1e6, ...
    lightSource2(1)*1e6, lightSource2(2)*1e6, sourceCenter(3)*1e6);
case 1
  %
  ...
  sourceDescription = sprintf('Rectangular aperture %.1f x %.1f [um] in z = 0', ...
    lightWidth * 1e6, lightHeight * 1e6);
case 2
  %
...
targetWidth = 150e-6; 
targetZ = 100e-6;
targetCenter = [-.0e-6, .0e-6, targetZ];
...
% filteringMethodA = 'none';

propagationMethodB = '2fft';
kernelTypeB = 'Exact';
% kernelTypeB = 'Fresnel';
filteringMethodB = 'localExact';
% filteringMethodB = 'localRect';
% filteringMethodB = 'none';
filteringMethodB = 'angular';
% filteringMethodB = 'angularLocalExact';
% filteringMethodB = 'angularLocalRect';
