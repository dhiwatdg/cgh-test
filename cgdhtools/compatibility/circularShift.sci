...
//
// Scilab equivalent of the Octave/MATLAB function circshift
//
// Inputs:
// m
//  - 2D matrix
...
//
// TODO
// Try to make a faster implementation
function out = circularShift(m, rowShift, colShift)
  rows = size(m, 1);
  cols = size(m, 2);
  rowShift = pmodulo(rowShift, rows);
  colShift = pmodulo(colShift, cols);
