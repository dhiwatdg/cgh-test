...
function colorbarLimits = displayImage(x, y, img, normalize, displayImages, fileName)
  ...
  if (isequalString(normalize, 'auto'))
    if (size(img, 3) == 3)
      % RGB images
      normalize = 'none';
    elseif (any(~isreal(img))) %% SCILAB %%     elseif (or(~isreal(img)))
      normalize = 'complex';
    elseif (all(img >= 0))     %% SCILAB %%     elseif (and(img >= 0))
      normalize = 'positive';
    else
      normalize = 'symmetric';
    end
  ...
  if (isequalString(normalize, 'complex'))
    ...
    workingImg(:,:,3) = inPart;
    workingImg = workingImg ./ max(1e-6, max(workingImg, [], 3)); %% SCILAB %% // empty
    %% SCILAB %%     d = max(1e-6, workingImg(:,:,1), workingImg(:,:,2), workingImg(:,:,3));
    %% SCILAB %%     workingImg(:,:,1) = workingImg(:,:,1) ./ d;
    %% SCILAB %%     workingImg(:,:,2) = workingImg(:,:,2) ./ d;
    %% SCILAB %%     workingImg(:,:,3) = workingImg(:,:,3) ./ d;
  ...
  if (displayImages)
    if (isequalString(normalize, 'complex') || ...
      ...
      if (isequalString(normalize, 'complex'))
        image(x, y, workingImg);         %% SCILAB %%         Matplot1(flipud(workingImg), [x(1), y(1), x($), y($)]);
        colormap(colormapComplex());     %% SCILAB %%         gcf().color_map = colormapComplex();
      elseif (isequalString(normalize, 'complexReIm'))
        image(x, y, workingImg);         %% SCILAB %%         Matplot1(flipud(workingImg), [x(1), y(1), x($), y($)]);
        colormap(colormapComplexReIm()); %% SCILAB %%         gcf().color_map = colormapComplexReIm();
      elseif (isequalString(normalize, 'phase'))
        imagesc(x, y, 360*workingImg);   %% SCILAB %%         Matplot1(flipud(workingImg*256), [x(1), y(1), x($), y($)]);
        colormap(gray());                %% SCILAB %%         gcf().color_map = graycolormap(256);
      end
      colorbarLimits = [0, 360];
      caxis(colorbarLimits);      %% SCILAB %%       // empty
    else
                                  %% SCILAB %%       if (size(img, 3) == 1)  // grayscale image
                                  %% SCILAB %%         colmapSize = 256;
                                  %% SCILAB %%       else                    // RGB image
                                  %% SCILAB %%         colmapSize = 1;
                                  %% SCILAB %%       end
      imagesc(x, y, img, limits); %% SCILAB %%       Matplot1(flipud(workingImg) * colmapSize, [x(1), y(1), x($), y($)]);
      colormap(gray());           %% SCILAB %%       gcf().color_map = graycolormap(colmapSize);
    end
                                  %% SCILAB %%     gca().data_bounds = [x(1), y(1); x($), y($)];
                                  %% SCILAB %%     gca().axes_visible = ['on', 'on']
    set(gca, 'YDir', 'normal');   %% SCILAB %%     // empty
    set(gca, 'tickdir', 'out');   %% SCILAB %%     // empty
  end
end
