...
function [target, x, y, kernel, kernelX, kernelY] = ...
  ...
  if (displayKernel > 0 || ~isequalString(saveKernel, ''))
    ...
    if (displayKernel > 0)
      ...
      drawLine([kernelX(m), kernelX(m)]*1e3, [kernelY(1), kernelY($)]*1e3, 'red', 2);
      drawLine([kernelX(1), kernelX($)]*1e3, [kernelY(n), kernelY(n)]*1e3, 'red', 2);
      gridOn([1, 1, 0], 0.6);
      subplot(2, 1, 2);
    end
    ...
    if (displayKernel > 0)
      ...
      xc = (kernelX(1) + kernelX($)) / 2;
      yc = (kernelY(1) + kernelY($)) / 2;
      
      // Local frequency estimate
      tmpX = [kernelX(1), kernelX($),         xc,           xc];
      tmpY = [        yc,           yc, kernelY(1), kernelY($)];
      tmpR = sqrt(tmpX.^2 + tmpY.^2 + z0^2);
      lxMin = kernelX(1) / (lambda * tmpR(1));
      lxMax = kernelX($) / (lambda * tmpR(2));
      lyMin = kernelY(1) / (lambda * tmpR(3));
      lyMax = kernelY($) / (lambda * tmpR(4));
      
      // Draw bounds
      if (lxMin >= kernelFX(1))
        drawLine([lxMin, lxMin]*1e-3, [kernelFY(1), kernelFY($)]*1e-3, 'red', 2);
      end
      if (lxMax <= kernelFX($))
        drawLine([lxMax, lxMax]*1e-3, [kernelFY(1), kernelFY($)]*1e-3, 'red', 2)
      end
      if (lyMin >= kernelFY(1)) 
        drawLine([kernelFX(1), kernelFX($)]*1e-3, [lyMin, lyMin]*1e-3, 'red', 2)
      end
      if (lyMax <= kernelFY($))
        drawLine([kernelFX(1), kernelFX($)]*1e-3, [lyMax, lyMax]*1e-3, 'red', 2)
      end
      gridOn([1, 1, 0], 0.6);
    end
  end
