  ...
  if (displayKernel > 0 || ~isequalString(saveKernel, ''))
    ...
    if (displayKernel > 0)
      ...
      if (lfxRange(1) >= kernelFX(1))
        drawLine([lfxRange(1), lfxRange(1)]*1e-3, [kernelFY(1), kernelFY(end)]*1e-3, 'red', 2);
      end
      if (lfxRange(2) <= kernelFX(end))
        drawLine([lfxRange(2), lfxRange(2)]*1e-3, [kernelFY(1), kernelFY(end)]*1e-3, 'red', 2);
      end
      if (lfyRange(1) >= kernelFY(1)) 
        drawLine([kernelFX(1), kernelFX(end)]*1e-3, [lfyRange(1), lfyRange(1)]*1e-3, 'red', 2);
      end
      if (lfyRange(2) <= kernelFY(end))
        drawLine([kernelFX(1), kernelFX(end)]*1e-3, [lfyRange(2), lfyRange(2)]*1e-3, 'red', 2);
      end
      gridOn([1, 1, 0], 0.6);
    end
  end    
