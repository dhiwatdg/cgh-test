...
if (drawLimitLines)
  ...
  if (lxMin > x(1))  
    % left border
    drawLine([lxMin, lxMin], [y(1), y(end)], 'green',  2);
  ...
  if (lxMax < x(end))
    % right border
    drawLine([lxMax, lxMax], [y(1), y(end)], 'red',    2);
  ...
  if (lyMin > y(1))
    % bottom border
    drawLine([x(1), x(end)], [lyMin, lyMin], 'blue',   2);
  ...
  if (lyMax < y(end))
    % top border
    drawLine([x(1), x(end)], [lyMax, lyMax], 'yellow', 2);
