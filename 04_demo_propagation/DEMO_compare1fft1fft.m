...
% Fourier transform based based (1fft) methods. 
%
% A source "image" is created in the plane z = 0. Subsequently, it is propagated 
% to the plane z = targetZ to a rectangular area "target". The distance
% targetZ is set quite large in this demo to show that the Fresnel and
% the Fraunhofer approximations give comparable results. Make it
% significantly shorter to see differences between near-field (Fresnel) and
...
% and DEMO_compare3fft2fft for convolution based methods.
%
%
% Generally, the propagation using 1fft method is calculated as
...
% The auxiliary functions (kernels) are displayed in Figures 2 and 3.
...
%   'compare1fft1fft_sourceAbs.png'.
% The propagations are saved to 
%   'compare1fft1fft_targetA.png', 
%   'compare1fft1fft_targetB.png'
%
% The kernels (i.e. the auxiliary functions used in the propagation)
% are saved (in case displayKenels is true) to 
%   'compare1fft1fft_kernelA_XiEta.png'
%   'compare1fft1fft_kernelA_XY.png'
%   'compare1fft1fft_kernelB_XiEta.png'
%   'compare1fft1fft_kernelB_XY.png'
%
% ---------------------------------------------
%
%  CGDH TOOLS
...
% Set target parameters automatically
targetSize = [];
targetDeltaYX = [];

% Propagation methods
propagationMethodA = '1fft';
kernelTypeA = 'Fresnel';
filteringMethodA = 'localExact';

propagationMethodB = '1fft';
kernelTypeB = 'Fraunhofer';
filteringMethodB = 'localExact';

% Set variables for displaying/saving kernels
if (displayKernel)
  fileNameKernelA = 'compare1fft1fft_kernelA';
  fileNameKernelB = 'compare1fft1fft_kernelB';
  figureKernelA = 2;
  figureKernelB = 3;
else
  fileNameKernelA = '';
...
targetAWidth = sourceSize(2)*targetADeltaYX(2);
targetAHeight = sourceSize(1)*targetADeltaYX(1);
...
limits = displayImage(sourceX*1e3, sourceY*1e3, abs(source), 'positive', 1, 'compare1fft1fft_sourceAbs.png');
xlabel('x [mm]');
ylabel('y [mm]');
...
subplot(3, 1, 2);
limits = displayImage(xA*1e3, yA*1e3, abs(targetA).^2, 'positive', 1, 'compare1fft1fft_targetAIntensity.png');
xlabel('x [mm]');
ylabel('y [mm]');
...
subplot(3, 1, 3);
limits = displayImage(xB*1e3, yB*1e3, abs(targetB).^2, 'positive', 1, 'compare1fft1fft_targetBIntensity.png');
xlabel('x [mm]');
ylabel('y [mm]');
